...
class DepthPrediction(Node):
    MODEL_REGISTRY = {
        ...
        'depth_anything': (load_depthanything_metric_depth_model, depthanything_predict_depth),
    }

    def __init__(self, in_prefix: str = "", out_prefix: str = None, model_name: str = 'zoedepth', model_path: str = "isl-org/ZoeDepth", batch_size: int = 8, device: Union[str, torch.device] = 'cuda'):
    ...
    def __call__(self, data: Dict[str, torch.Tensor], pipe=None):
        ...
        model = self.get_lazy_component(self.model_name, self._model_fn, init_kwargs={'model_path': self.model_path, 'device': self.device}, pipe=pipe)
        depth_tensor = []
...
class NormalizeDepth(Node):
    def __init__(self, in_key: str = "depth", out_key: str = "depth", by: Literal['median', 'mean', 'min', 'max'] = 'median'):
        super().__init__()
        self.in_key = in_key
        self.out_key = out_key
        assert by in ['median','mean','min','max'], f"Invalid by option: {by}"
        self.by = by
    ...
    def __call__(self, data: Dict[str, torch.Tensor], pipe=None):
        ...
        if self.by =='median':
            ...
            data[self.out_key] = data[self.in_key] / data[self.in_key].flatten(-2).mean(dim=-1)[0][..., None, None]
        elif self.by =='min':
            data[self.out_key] = data[self.in_key] / data[self.in_key].flatten(-2).min(dim=-1)[0][..., None, None]
        elif self.by =='max':
            data[self.out_key] = data[self.in_key] / data[self.in_key].flatten(-2).max(dim=-1)[0][..., None, None]
        return data
